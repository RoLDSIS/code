...
for (subj in cohort) {
    ...
    sol <- roldsis (folds$x, psycho.resp)
    dir <- sol$direction
    proj <- sol$projection
    signals <- apply (proj, 1,
                      function (x) vec.to.signal (x, dwt.length))

    ## ** Generate the scalogram figure
    pdf (file.path (figures.dir, sprintf ("cv-direction-S%02d.pdf", subj)),
         width = 5 * 1.2, height = 3 * 1.2)
    ...
    plot (t, x, type = "l", bty = "n", lwd = 2, las = 1, xaxt = "n",
          xlab = "", yaxt = "n", ylab = "", xlim = c (0, max (t)) + time.shift)
    ...
    pdf (file.path (figures.dir,
                    sprintf ("cv-projections-S%02d.pdf", subj)),
         width = 6, height = 3)

    par (mar = c (5, 4, 0, 0) + 0.1)

    y.lim <- c (min (signals), max (signals))
